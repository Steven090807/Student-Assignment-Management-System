    ...
    <div style="margin-top: 30px; margin-left: -30%">
     <div class="search-container">
      ...
      <div class="dropdown dropdown-space" id="MostRecent">
          <button id="recentBtn" class="btn btn-secondary dropdown-toggle dropdown-btn" type="button" data-bs-toggle="dropdown">
            Most Recent
          </button>
      </div>
      <div id="classContainer" class="row mt-4"></div>
     </div>
    </div>
  </div>
</div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="row">
                ...
                $stmt = $conn->prepare("SELECT * FROM `class_student` WHERE `student_name` = ?");
                ...
                if ($result->num_rows == 0) {
                    ...
                    <div id="nothing" style="text-align: center; margin-top: 120px;">
                        ...
                        <p style="color: gray; margin-top: 10px;">Haven&#39;t joined the course yet</p>
                    </div>';
                } else {
                            ...
                            <div class="custom-panel-all" onclick="window.location.href=\'classwork_01.php?className=' . urlencode($row['class_name']) . '\'">
                                ...
                                <div class="custom-panel">
                                    <div class="custom-panel-top">
                                        ...
                                        <div class="avatar" style="background-color: '.htmlspecialchars($row['color']).';">Mr</div>
                                    </div>
...
<footer>
    <button class="btn-circle" id="openPopup">
        ...
        <span class="text">Join a class</span>
    </button>
</footer>
<div class="popup-overlay" id="popup">
  <div class="add-class-btn">
    <button type="button" class="btn-close" id="closePopup" aria-label="Close">X</button>
    <h2><b>+ Join Class</b></h2><hr>
    <div class="form-group">
      <label for="title">Class Name</label>
      <input type="text" id="title" placeholder="Enter your class name to join" maxlength="16" required>
...
document.addEventListener('click', function (event) {
  if (event.target.classList.contains('dropdown-item') && event.target.textContent.trim() === 'Delete') {
    event.preventDefault();
    event.stopPropagation();

    if (confirm("Are you sure you want to delete this class?")) {
      const panel = event.target.closest('.col-md-4');
      const className = panel.querySelector('.class-name').textContent.trim();
      const studentName = "<?php echo $_SESSION['student_name']; ?>"; // get current logged-in student
      ...
      fetch('delete_class.php', {
        ...
        body: `class_name=${encodeURIComponent(className)}&student_name=${encodeURIComponent(studentName)}`
      })
      .then(response => response.text())
      .then(data => {
        if (data === 'success') {
